...
# get rid of whitespace in var names + reorder
# data |> colnames()
library(janitor)
data <- data |>
  clean_names(sep_out=".")
...
age_data <- age_data %>%
  mutate(age_group = cut(q.demos.age, 
                         breaks = c(18, 25, 35, 45, 55, 65, 100), 
                         labels = c("18-24","25-34","35-44","45-54","55-64","65+"),
                         right = FALSE))

ggplot(age_data, aes(x = q.amazon.use.hh.size, 
                     y = purchase.price.per.unit * quantity, 
                     fill = factor(q.amazon.use.howmany))) +
  geom_boxplot() +
...
## Preprocessing
```{r}
save(data, file="data/regression/joined_data.RData")
