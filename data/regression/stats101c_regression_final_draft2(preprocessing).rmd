...
numeric_data <- data %>%
  dplyr::select(Purchase.Price.Per.Unit, Quantity, Q.amazon.use.howmany, 
...
```{r, fig.width=10, fig.height=6}
behavior_data <- data %>%
  dplyr::select(Q.substance.use.cigarettes,
...
So our main goal is to predict the household size of a customer using their purchasing behavior.

### Cleaning data: dropping unecessary columns, recoding values, imputing missing values.

Since our main goal is to predict the household size of a customer using their purchasing behavior, we can drop several columns:
-Q.demos.state
...
-Q.census.use
-Q.amazon.use.hh.size (we already have the household number so we're keeping that)
...
```{r}
data <- data %>%
  dplyr::select(-Q.demos.state, -Shipping.Address.State, -ASIN.ISBN..Product.Code.,
                -Order.Date, -Q.life.changes, -Q.sell.YOUR.data, -Q.small.biz.use,
                -Q.research.society, -test, -Q.demos.hispanic, -Q.demos.race, -Q.demos.education,
                -Q.substance.use.cigarettes, -Q.substance.use.marijuana, -Q.substance.use.alcohol,
                -Q.personal.diabetes, -Q.personal.wheelchair, -Q.sell.consumer.data,
                -Q.census.use, -Q.amazon.use.hh.size, -Q.amazon.use.howmany) 
str(data)

```

```{r}
str(unique(data$Q.demos.age))
str(unique(data$Q.demos.income), vec.len = Inf)
str(unique(data$Q.demos.gender))
str(unique(data$Q.sexual.orientation))
str(unique(data$Q.amazon.use.howmany))
str(unique(data$Q.amazon.use.hh.size))
str(unique(data$Q.amazon.use.hh.size.num))
str(unique(data$Q.amazon.use.how.oft))
...
data <- data %>%
  mutate(
    # Gender: keep as factor (categorical)
    Q.demos.gender = factor(
      Q.demos.gender,
      levels = c("Male", "Female", "Other", "Prefer not to say")
    ),

    # Sexual orientation: keep as factor (categorical)
    Q.sexual.orientation = factor(
      Q.sexual.orientation,
      levels = c("heterosexual (straight)", "LGBTQ+", "prefer not to say")
      # NA stays NA
    ),
    ...
    # Amazon use frequency: ordered factor (categorical)
    Q.amazon.use.how.oft = factor(
...
Missing values in numeric variables are imputed using regression trees, while missing values in categorical variables are imputed using classification trees, allowing missing entries to be filled using patterns learned from the data rather than simple averages. After imputation, all variables are recombined into a complete feature set, and categorical variables are transformed into numerical indicators through one-hot encoding. The resulting fully numeric and complete dataset is then used to train an XGBoost regression model.

However, variables that do not fall into either category—such as Survey.ResponseID, Title, and Category are passed through unchanged. Thus they still have **missing values**.

## Candidate models / Model evaluation / Tuning
```{r}
# Create learner
lrn_xgb <- as_learner(graph_xgb)
```
