...
```{r}
#| label: feature_creation

# total.orders and n.distinct.categories
train_data <- train_data |> 
  mutate(total.orders = total.spent / avg.order.price) |> 
  rowwise() |> 
  mutate(
    n.distinct.categories = sum(c_across(starts_with("freq_")) > 0)
  ) |> ungroup()
...
```{r}
#| label: regtree
lrn_rpart <- lrn("regr.rpart") # default
lrn_rpart$train(tsk_hhsize)
lrn_rpart$model |> rpart.plot::rpart.plot()
...
glrn_rpart_encode <- as_learner(factor_pipeline %>>% lrn_rpart)

instance <- ti(
  task = tsk_hhsize,
  learner = glrn_rpart_encode,
  resampling = rsmp("cv", folds = 5),
  measure = msr("regr.rmse"),
  terminator = trm("none")
)
