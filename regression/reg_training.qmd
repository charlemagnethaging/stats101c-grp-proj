...
lrn_rf <- as_learner(
  # po("encode", method = "one-hot") %>>% try no encoding?
  lrn("regr.ranger",
      splitrule = "extratrees",
      respect.unordered.factors = "order", # internal encoding
      num.trees = 600, 
...
tune_rf2 <- lrn_rf$clone()
tune_rf2$param_set$values <- instance_rf$result_learner_param_vals

hhsize_split <- partition(tsk_hhsize, ratio=0.8)
tune_rf2$train(tsk_hhsize, row_ids = hhsize_split$train)
preds <- tune_rf2$predict(tsk_hhsize, row_ids = hhsize_split$test)
...
tsk_hhsize_test <- as_task_regr(test_data, target = "q.amazon.use.hh.size.num", id="hh_size_test")
tsk_hhsize_test$set_col_roles("survey.response.id", "name")
sub_5 <- tune_rf2$predict(tsk_hhsize_test)
sub_5_df <- cbind(tsk_hhsize_test$row_names, sub_5$response) |> 
  as_tibble() |> 
  select(!row_id) |> 
  # mutate(V2 = round(V2)) |> don't round?
  rename(Survey.ResponseID = row_name, Q.amazon.use.hh.size.num = V2)

write_csv(sub_5_df, "submissions/charlie_submission_5.csv")
```
