...
## create category frequency per id
...
purchases <- purchases |>
	...
	summarize(order.price = sum(purchase.price.per.unit * quantity)) |> # sum prices in a day
	group_by(survey.response.id) |>
	summarize(total.spent = sum(order.price), avg.order.price = mean(order.price), .groups='drop')
...
```{r old}
#| label: superceded preprocessing
#| eval: false
