...
# lime visual
# set up a predictor
predictor_s = Predictor$new(
  model = tuned_lrn_xgboost1,
  data = tsk_housing$data(cols = setdiff(colnames(ames), "Sale_Price")),
y = tsk_ames_s$truth())
# fit local model, k is number of predictors
lim1 = LocalModel$new(predictor_s, x.interest = predictor_s$data$X[1,], k = 6)
41/45
